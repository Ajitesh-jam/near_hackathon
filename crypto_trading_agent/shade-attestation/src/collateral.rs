...
impl JsonSchema for Collateral {
    fn schema_name() -> String {
        "Collateral".into()
    }
    fn json_schema(_schema_gen: &mut schemars::gen::SchemaGenerator) -> Schema {
        SchemaObject {
...
impl BorshSchema for Collateral {
    fn declaration() -> borsh::schema::Declaration {
        "Collateral".into()
    }
    fn add_definitions_recursively(definitions: &mut borsh::schema::BorshSchemaContainer) {
        <QuoteCollateralV3 as BorshSchema>::add_definitions_recursively(definitions);
        definitions.insert(Self::declaration(), Definition::Struct {
            fields: Fields::UnnamedFields(alloc::vec![QuoteCollateralV3::declaration()]),
        });
    }
}
