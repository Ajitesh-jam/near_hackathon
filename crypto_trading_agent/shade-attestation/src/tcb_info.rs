...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, BorshSerialize, BorshDeserialize, JsonSchema, BorshSchema)]
pub struct TcbInfo {
...
impl<const N: usize> JsonSchema for HexBytesOrEmpty<N> {
    fn schema_name() -> String {
        alloc::format!("HexBytesOrEmpty_{}", N)
    }
    fn json_schema(_schema_gen: &mut schemars::gen::SchemaGenerator) -> Schema {
        // Option<HexBytes<N>> serializes as string or empty; represent as string in schema
        SchemaObject {
