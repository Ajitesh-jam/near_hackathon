...
export function createApi(): Hono {
  ...
  app.post("/chat", async (c) => {
    ...
    try {
      ...
      if (message)
        console.log("[API] /chat message length:", String(message).length);
  ...
  app.get("/notifications", async (c) => {
    ...
    try {
      ...
      const items = all.map((n) => ({
        ...
        time_of_occur:
          n.time_of_occur ?? (n as Record<string, unknown>).time_of_occur,
        description:
          n.description ?? (n as Record<string, unknown>).description,
        which_tool_to_call:
          n.which_tool_to_call ??
          (n as Record<string, unknown>).which_tool_to_call,
  ...
  app.post("/notifications/:id/respond", async (c) => {
    ...
    try {
      ...
      if (action !== "approve" && action !== "reject" && action !== "dismiss") {
        return c.json(
          formatError(
            new Error("action must be 'approve', 'reject', or 'dismiss'"),
          ),
          400,
        );
      }
      ...
      console.log(
        "[API] /notifications/:id/respond error:",
        error instanceof Error ? error.message : String(error),
      );
      return c.json(formatError(error), 500);
    }
  ...
  app.get("/scheduled-events", async (c) => {
    ...
    try {
      ...
      const items = all.map((e) => ({
        ...
        time_of_occur:
          e.time_of_occur ?? (e as Record<string, unknown>).time_of_occur,
        description:
          e.description ?? (e as Record<string, unknown>).description,
        which_tool_to_call:
          e.which_tool_to_call ??
          (e as Record<string, unknown>).which_tool_to_call,
  ...
  app.post("/scheduled-events/:id/reject", async (c) => {
    ...
    try {
      ...
      console.log(
        "[API] /scheduled-events/:id/reject error:",
        error instanceof Error ? error.message : String(error),
      );
      return c.json(formatError(error), 500);
    }
