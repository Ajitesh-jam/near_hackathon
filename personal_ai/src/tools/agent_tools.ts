...
export async function save_personal_info(args: {
  ...
  if (typeof field !== "string" || !field.trim()) {
    return {
      success: false,
      error:
        "Field is required. Use secrets for passwords/credentials; memory, notes, interests, hobbies, dailyJournal, moodLogs, goals_short; or a custom key for other data.",
    };
  }
...
async function schedule_event(args: {
  ...
  try {
    const event = {
      ...
      which_tool_to_call:
        typeof which_tool_to_call === "string" ? which_tool_to_call : "pay",
...
async function pay(args: {
  ...
  const recipient =
    args?.recipient ??
    (args?.arguments as Record<string, unknown>)?.recipient ??
    "unknown";
  const amount =
    args?.amount ?? (args?.arguments as Record<string, unknown>)?.amount ?? "0";
  const address =
    args?.address ??
    (args?.arguments as Record<string, unknown>)?.address ??
    "unknown address";
  ...
  try {
    ...
    console.log(
      `[PAY]  payout of ${yoctoAmount} yoctoNEAR to ${rec} at address ${addr}`,
    );
...
export const toolMap: Record<string, (args: unknown) => Promise<unknown>> = {
  ...
  save_personal_info: (args) =>
    save_personal_info(
      args as { field?: string; value?: unknown; reason?: string },
    ),
  schedule_event: (args) =>
    schedule_event(
      args as {
